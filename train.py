...
class DatasetLazyIter(object):
    ...
    def __init__(self, datasets, fields, batch_size, batch_size_fn,
                 device, is_train, src_data_type, tgt_data_type):
        ...
        if src_data_type == 'trigrams' and tgt_data_type == 'words':
            self.iterator = onmt.io.IO.SourceCharactersOrderedIterator
        elif src_data_type == 'words' and tgt_data_type == 'characters':
            self.iterator = onmt.io.IO.TargetCharactersOrderedIterator
        elif src_data_type in ['trigrams', 'characters'] and tgt_data_type == 'characters':
            self.iterator = onmt.io.IO.BothCharactersOrderedIterator
        else:
            self.iterator = onmt.io.OrderedIterator
...
def make_dataset_iter(datasets, fields, opt, is_train=True):
    ...
    return DatasetLazyIter(datasets, fields, batch_size, batch_size_fn,
                           device, is_train, opt.src_data_type, opt.tgt_data_type)
...
def train_model(model, fields, optim, src_data_type, tgt_data_type, model_opt):
    ...
    trainer = onmt.Trainer(model, train_loss, valid_loss, optim,
                           trunc_size, shard_size, opt.src_data_type, opt.tgt_data_type, 
...
def load_fields(dataset, src_data_type, tgt_data_type, checkpoint):
    if checkpoint is not None:
        print('Loading vocab from checkpoint at %s.' % opt.train_from)
        fields = onmt.io.load_fields_from_vocab(
            checkpoint['vocab'], src_data_type)
    else:
        fields = onmt.io.load_fields_from_vocab(
    ...
    elif src_data_type in ['trigrams', 'characters'] and tgt_data_type == 'words':
...
def main():
    ...
    src_data_type = first_dataset.src_data_type
    tgt_data_type = first_dataset.tgt_data_type

    print("Source data type: " + src_data_type)
    print("Target data type: " + tgt_data_type)
