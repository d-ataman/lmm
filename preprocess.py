...
def build_save_text_dataset_in_shards(src_corpus, tgt_corpus, fields,
    ...
    This is currently only for data_type=='words' or 'trigrams'.
    ...
    while not src_iter.hit_end():
        ...
        dataset = onmt.io.TextDataset(
                ...
                src_data_type=opt.src_data_type, 
                tgt_data_type=opt.tgt_data_type)
...
def build_save_dataset(corpus_type, fields, opt):
    ...
    # Currently we only do preprocess sharding for corpus: data_type=='words' or 'words'.
    if opt.src_data_type in ['words', 'trigrams'] and opt.tgt_data_type in ['words', 'characters']:
    ...
    dataset = onmt.io.build_dataset(
                fields, 
                opt.src_data_type, opt.tgt_data_type, 
...
def main():
    ...
    src_nfeats = onmt.io.get_num_features(opt.src_data_type, opt.train_src, 'src')
    tgt_nfeats = onmt.io.get_num_features(opt.tgt_data_type, opt.train_tgt, 'tgt')
    print(" * number of source features: %d." % src_nfeats)
    print(" * number of target features: %d." % tgt_nfeats)

    print("Building `Fields` object...")
    fields = onmt.io.get_fields(opt.src_data_type, src_nfeats, tgt_nfeats)
