...
from .Wordbatch import SourceWordIterator, TargetWordIterator, BothWordIterator
...
def get_fields(data_type, n_src_features, n_tgt_features):
    """
    Args:
        data_type: type of the source input. Options are [words|characters|trigrams|img|audio].
    ...
    """
    if data_type in ['words', 'trigrams', 'characters']:
...
def load_fields_from_vocab(vocab, data_type="words"):
    """
...
def get_num_features(data_type, corpus_file, side):
    ...
    Args:
        data_type (str): type of the source input.
            Options are [words|trigrams|characters|img|audio].
...
def make_features(batch, side, data_type='words'):
    """
    Args:
...
def build_dataset(fields, src_data_type, tgt_data_type, src_path, tgt_path, src_dir=None,
    ...
    src_examples_iter, num_src_feats = \
        _make_examples_nfeats_tpl(src_data_type, src_path, src_dir,
    ...
    if src_data_type in ['words', 'trigrams', 'characters']:
...
def convert_into_characters(inputtuple):
    sen = []
    for w in inputtuple:
        char = '^^'
        sen.append(char)
...
def build_vocab(train_dataset_files, fields, src_data_type, tgt_data_type, share_vocab,
    ...
    Args:
        ...
        data_type: "words", "trigrams", "characters", "img" or "audio"?
    ...
    for path in train_dataset_files:
        ...
        for ex in dataset.examples:
            for k in fields:
                ...
                if val is not None:
                    if src_data_type == 'trigrams' and k == "src":
                        val = convert_into_trigrams(val)
                    elif src_data_type == 'characters' and k == "src":
                        val = convert_into_characters(val)
                    if tgt_data_type == 'characters' and k == "tgt":
                        #val = convert_into_trigrams(val)
                        val = split_bpe(val)
...
class SourceCharactersOrderedIterator(SourceWordIterator):
    def create_batches(self):
        if self.train:
            self.batches = torchtext.data.pool(
                self.data(), self.batch_size,
...
class TargetCharactersOrderedIterator(TargetWordIterator):
    def create_batches(self):
        if self.train:
            self.batches = torchtext.data.pool(
                self.data(), self.batch_size,
...
class BothCharactersOrderedIterator(BothWordIterator):
    def create_batches(self):
        if self.train:
            self.batches = torchtext.data.pool(
                self.data(), self.batch_size,
